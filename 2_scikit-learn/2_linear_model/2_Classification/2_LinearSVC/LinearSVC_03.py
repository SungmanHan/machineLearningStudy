...
# L1 제약 조건을 사용하는 경우 정규화 처리를 진행한 
# 데이터에 대해서 성능이 감소할 수 있습니다.
...
# 아래의 예는 제약조건을 L1으로 변경한 상태에서 
# 모델을 테스트하는 예제
for C, marker in zip([0.01, 1, 100], ['o', '^', 'v']):
    svm_model_l1 = LinearSVC(
            C=C, penalty="l1", dual=False, 
            max_iter=100000).fit(X_train, y_train)
    print("C={:.3f} 인 l1 LinearSVC 분류의 훈련 정확도: {:.2f}".format(
          C, svm_model_l1.score(X_train, y_train)))
    print("C={:.3f} 인 l1 LinearSVC 분류의 테스트 정확도: {:.2f}".format(
